...
const VERSIONE = 'negozio-2026-09-16a';
const SHELL = [
  './Negozio-v2.html',
  './Cassa.html',
...
self.addEventListener('fetch', e => {
  ...
  e.respondWith(caches.open(VERSIONE).then(async c => {
    const inCache = await c.match(e.request, { ignoreSearch: true });
    const dallaRete = fetch(e.request).then(r => { if (r && r.ok) c.put(e.request, r.clone()); return r; }).catch(() => null);
